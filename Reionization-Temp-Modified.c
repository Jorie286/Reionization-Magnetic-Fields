/* Here we modificatied code by frankelzeng, the original code does not return the tau_ values but we will return them for our equations*/
...
int main(int argc, char **argv) {
  ...
  for(j=0; j<NGRID; j++) {
    y1H[j] = 1-1.9087440553e-4;
    y1He[j] = 1.-1.e-10;
    EH[j] = 1.3788981353 * 1.5*(1.-y1H[j]+ABUND_HE*(1.- 1*y1He[j]))/RYD_K;
    EHI[j] = 1.3788981353 * 1.5*y1H[j]/RYD_K;
    EHeI[j] = 1.3788981353 * 1.5*y1He[j]*ABUND_HE/RYD_K;
    EHII[j] = 1.3788981353 * 1.5*(1.-y1H[j])/RYD_K;
    EHeII[j] = 1.3788981353 * 1.5*(1.-y1He[j])*ABUND_HE/RYD_K;
  }
