...
def get_Slm(yH, tauHdat, tauHedat, fracflux, i, velocity):
    ...
    if r_H<calc_params.N_NU:
        ...
        Slm_H = -(1/(8*math.pi))*(math.sqrt((16*math.pi)/5))*calc_params.n_H*F*A_j_H*(tauHdat[r_H,i]/(tauHdat[r_H,i]+tauHedat[r_H,i]))*(calc_params.m_e/(velocity*delta_E_H))
    else:
        Slm_H = 0

    if r_He<calc_params.N_NU:
        A_j_He = fracflux[r_He]*math.exp(-np.sum(tautot[r_He,:i]))*((-np.expm1(-tautot[r_He,i]))/calc_params.DNHI)
        Slm_He = -(1/(8*math.pi))*(math.sqrt((16*math.pi)/5))*calc_params.n_H*F*A_j_He*(tauHedat[r_He,i]/(tauHdat[r_He,i]+tauHedat[r_He,i]))*(calc_params.m_e/(velocity*delta_E_He))
    else:
        Slm_He = 0
